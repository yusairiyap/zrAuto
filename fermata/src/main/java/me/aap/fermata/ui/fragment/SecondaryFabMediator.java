	...
	public boolean onLongClick(View v) {
		...
		menu.show(b -> {
			...
			if (MusicPlayer.isEnabled() &&
					(Action.get(a.getPrefs().getIntPref(MainActivityPrefs.FAB2_ACTION)) != Action.PLAY_AS_MUSIC)) {
				b.addItem(R.id.fab_use_play_as_music, R.drawable.music, R.string.fab_use_play_as_music)
						.setHandler(item -> {
							a.getPrefs().applyIntPref(MainActivityPrefs.FAB2_ACTION, Action.PLAY_AS_MUSIC.ordinal());
							updateIcon((FloatingButton) v);
							return true;
						});
			}
